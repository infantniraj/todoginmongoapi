...
func NewUserUpload(c *gin.Context) {
	fmt.Println("Inside")
	...
	if file.Filename != "" {
		err = c.SaveUploadedFile(file, UploadLocation+file.Filename)
		...
		c.JSON(http.StatusOK, gin.H{
			...
			"message": "Sorry.There is a problem with file upload.Please try again",
		})
	}
	if err != nil {
		log.Fatal(err)
